...
SAGA requires raster data input in the format `.sgrd` and `.sdat`. So before we can use it in the RSAGA workflow, we have to convert existing data first.
...
#| echo: true
#| eval: false
params <- list(DEM = "dem.sgrd", TWI ="twi.sgrd")
